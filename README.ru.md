...
случайной паузой от 15 до 30 секунд. Для осторожного первого реального запуска
укажите `--limit 1`:
...
Явный эквивалент настроек по умолчанию:
...
`--batch-size` остаётся доступным как псевдоним `--limit`.
